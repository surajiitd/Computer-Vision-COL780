...
imageFiles=os.listdir("../data/Liquor/img/")
imageFiles.sort()

template = cv2.imread("../data/Liquor/img/"+imageFiles[0])
height, width, _ = template.shape
template = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)
template_original = copy.deepcopy(template)
region = np.array([[256, 152], [329, 362]])   # Bolt
...
for img in range(1,len(imageFiles)):
    ...
    image = cv2.imread("../data/Liquor/img/"+imageFiles[img])
    ...
    cv2.waitKey(1)
    cv2.imwrite( "./lk_predicted/liquor/" +imageFiles[img], image_copy)
