...
imageFiles=os.listdir("../data/BlurCar2/img/")
imageFiles.sort()

template = cv2.imread("../data/BlurCar2/img/"+imageFiles[0])
height, width, _ = template.shape
template = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)
template_original = copy.deepcopy(template)
region = np.array([[227, 207], [349, 306]])   # Bolt
...
for img in range(1,len(imageFiles)):
    ...
    image = cv2.imread("../data/BlurCar2/img/"+imageFiles[img])
    image_copy = copy.deepcopy(image)
    image_copy_2 = copy.deepcopy(image)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    p_prev = p
    p = normalLucasKanade(image, template, region, p_prev ,threshold)
    ...
    cv2.waitKey(1)
    cv2.imwrite( "./pyr_lk_predicted/blurcar/" +imageFiles[img], image_copy)
